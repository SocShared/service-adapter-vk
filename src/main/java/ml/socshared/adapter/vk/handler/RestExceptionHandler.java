...
public class RestExceptionHandler extends ResponseEntityExceptionHandler {
    ...
    private ResponseEntity<RestApiError> buildErrorResponse(String message, HttpStatus httpStatus, ServletWebRequest webRequest) {
        return new ResponseEntity<>(new RestApiError(message, httpStatus, webRequest), httpStatus);
    }
    ...
    public ResponseEntity<RestApiError> handleVkException(ServletWebRequest webRequest, VKClientException exp) {
        log.error(exp.getErrorType().getErrorMsg());
        exp.printStackTrace();
        return buildErrorResponse(exp.getErrorType().getErrorMsg(), HttpStatus.BAD_REQUEST, webRequest);
    }
}
